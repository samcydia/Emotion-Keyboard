...
/// 命名空间常量
NSString *const SAMEmoticonNamespace = @"cn.sam";
/// 默认用户标识符
NSString *const SAMEmoticonDefaultUserIdentifier = @"DefaultUser";
/// 表情文件名
NSString *const SAMEmoticonFileName = @".emoticons.json";

@implementation SAMEmoticonManager {
    NSMutableArray <SAMEmoticon *> *_recentEmoticonList;
}
...
- (NSAttributedString *)emoticonStringWithString:(NSString *)string font:(UIFont *)font {
    ...
    for (NSTextCheckingResult *result in matches.reverseObjectEnumerator) {
        ...
        SAMEmoticon *emoticon = [self emoticonWithString:str];
        if (emoticon != nil) {
            NSAttributedString *emoticonString = [SAMEmoticonAttacSAMent emoticonStringWithEmoticon:emoticon font:font];
...
- (SAMEmoticon *)emoticonWithString:(NSString *)string {
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"chs == %@", string];
    SAMEmoticon *emoticon = nil;
    
    for (NSInteger i = 1; i < _packages.count; i++) {
        
        SAMEmoticonPackage *package = _packages[i];
...
- (NSInteger)numberOfPagesInSection:(NSInteger)section {
    SAMEmoticonPackage *package = _packages[section];
...
- (NSArray *)emoticonsWithIndexPath:(NSIndexPath *)indexPath {
    SAMEmoticonPackage *package = self.packages[indexPath.section];
...
#pragma mark - 最近使用表情
- (void)addRecentEmoticon:(SAMEmoticon *)emoticon {
    // 0. 表情计数 ++
    emoticon.times++;
    ...
    // 2. 排序
    [_recentEmoticonList sortUsingComparator:^NSComparisonResult(SAMEmoticon *obj1, SAMEmoticon *obj2) {
...
- (void)saveRecentEmoticonList {
    ...
    for (SAMEmoticon *emoticon in _recentEmoticonList) {
...
///
/// @return 最近表情数组
- (NSMutableArray <SAMEmoticon *>*)loadRecentEmoticonList {
...
- (NSString *)filePathForRecentEmoticon {
    ...
    return [[dir stringByAppendingPathComponent:self.userIdentifier] stringByAppendingString:SAMEmoticonFileName];
}
...
- (void)loadPackages {
    ...
    NSString *path = [[NSBundle SAM_emoticonBundle] pathForResource:@"emoticons.plist" ofType:nil];
    NSArray *array = [NSArray arrayWithContentsOfFile:path];
    
    // 2. 遍历数组，生成 packages 模型
    for (NSDictionary *dict in array) {
        [_packages addObject:[SAMEmoticonPackage packageWithDict:dict]];
    }
...
- (NSString *)userIdentifier {
    if (_userIdentifier == nil) {
        _userIdentifier = SAMEmoticonDefaultUserIdentifier;
    }
    return [NSString stringWithFormat:@"%@.%@", SAMEmoticonNamespace, _userIdentifier];;
}
